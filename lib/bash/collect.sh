...
collect() {
   ...
   if [ ! "$OPT_SYSTEM_ONLY" ]; then
      if [ "${OPT_PGSQL:-}" ]; then
         # PostgreSQL mode: kick off pg_gather at the very start of the
         # iteration. The mysql-specific collectors below are skipped.
   ...
   while [ $((curr_time - start_time)) -lt $OPT_RUN_TIME ]; do
      ...
      if [ ! "$OPT_SYSTEM_ONLY" ] && [ -z "${OPT_PGSQL:-}" ]; then
...
pgsql_env() {
   # Build a libpq env-var prefix for psql invocations. Keeping the password
   # in PGPASSWORD instead of on the command line avoids leaking it into
   # 'ps' output (matches the rationale of the MySQL password warning).
   local prefix=""
   [ -n "${OPT_PG_HOST:-}" ]     && prefix="$prefix PGHOST='${OPT_PG_HOST}'"
   [ -n "${OPT_PG_PORT:-}" ]     && prefix="$prefix PGPORT='${OPT_PG_PORT}'"
   [ -n "${OPT_PG_USER:-}" ]     && prefix="$prefix PGUSER='${OPT_PG_USER}'"
   [ -n "${OPT_PG_DATABASE:-}" ] && prefix="$prefix PGDATABASE='${OPT_PG_DATABASE}'"
   [ -n "${OPT_PG_SERVICE:-}" ]  && prefix="$prefix PGSERVICE='${OPT_PG_SERVICE}'"
   [ -n "${OPT_PG_PASSWORD:-}" ] && prefix="$prefix PGPASSWORD='${OPT_PG_PASSWORD}'"
   echo "$prefix"
}

collect_pgsql_data_one() {
   # Run pg_gather (gather.sql) once at the start of the iteration.
   # If a previous iteration's pg_gather is still running we skip and
   # write a warning into the current iteration's output file instead
   # of starting a duplicate psql process. The flag file lives under
   # OPT_DEST so it survives across collect() invocations.
   local flag_file="$OPT_DEST/pg_gather.running"
   local outfile="$d/$p-pg-gather.tsv"
   [ "${OPT_PG_GZIP:-}" ] && outfile="${outfile}.gz"

   if [ -f "$flag_file" ]; then
      local prev_pid="$(cat "$flag_file" 2>/dev/null)"
      if [ -n "$prev_pid" ] && kill -0 "$prev_pid" 2>/dev/null; then
         local skip_msg="pg_gather from a previous iteration (PID $prev_pid) is still running; skipping pg_gather for this iteration"
         warn "$skip_msg"
         ts "$skip_msg" >> "$outfile"
         return 0
      else
         # Stale flag file (process gone). Remove and proceed.
         rm -f "$flag_file"
      fi
   fi

   local env_prefix
   env_prefix="$(pgsql_env)"
   local psql_bin="${OPT_PG_PSQL:-$CMD_PSQL}"
   log "Running pg_gather ($OPT_PG_GATHER_SQL) -> $outfile"
   (
      if [ "${OPT_PG_GZIP:-}" ]; then
         eval "$env_prefix \"$psql_bin\" -X -f \"$OPT_PG_GATHER_SQL\"" | gzip > "$outfile"
      else
         eval "$env_prefix \"$psql_bin\" -X -f \"$OPT_PG_GATHER_SQL\"" > "$outfile"
      fi
      rm -f "$flag_file"
   ) &
   echo "$!" > "$flag_file"
